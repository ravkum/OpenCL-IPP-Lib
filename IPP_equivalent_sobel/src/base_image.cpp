...
int cppiGrayToRGB_8u_P1C3R(unsigned char *pSrc, unsigned int iSrcStep, unsigned char *pDst, unsigned int iDstStep, unsigned int iWidth, unsigned int iHeight)
{
#ifdef USE_IPP
    IppStatus ippStatus;
    IppiSize roiSize = {iWidth, iHeight};
    ippStatus = ippiDup_8u_C1C3R((const Ipp8u*)pSrc, iSrcStep, (Ipp8u*)pDst, iDstStep, roiSize);
    return (ippStsNoErr == ippStatus)? 0 : STS_ERR_FAILED;
#else
...
int cppiRGBToGray_8u_C3P1R(unsigned char *pSrc, unsigned int iSrcStep, unsigned char *pDst, unsigned int iDstStep, unsigned int iWidth, unsigned int iHeight, bool bBGR)
{
#ifdef USE_IPP
        IppStatus ippStatus;
    IppiSize roiSize = {iWidth, iHeight};
...
int cppiSwapChannes_C3(unsigned char *pSrc, unsigned int iSrcStep, unsigned char *pDst, unsigned int iDstStep, unsigned int iWidth, unsigned int iHeight)
{
#ifdef USE_IPP
    IppStatus ippStatus;
...
int Image::Alloc(unsigned int iWidth, unsigned int iHeight, unsigned int iSamples, unsigned int iSampleSize, unsigned int iBufferSize, unsigned int iStep)
{
   ...
   // m_pBuffer = (unsigned char*)malloc(m_iBufferSize, m_iBufferAlignment);
	 m_pBuffer = (unsigned char*)_aligned_malloc(m_iBufferSize, m_iBufferAlignment);
    if(!m_pBuffer)
        return -1;
...
int Image::Alloc(unsigned int iWidth, unsigned int iHeight, ImageColor color, unsigned int iSampleSize, unsigned int iBufferSize, unsigned int iStep)
{
    m_color    = color;
    m_iSamples = colorChannels[m_color];
...
int Image::Alloc(unsigned int iBufferSize, unsigned int iStep)
{
...
int Image::Read(const char *pFileName, ImageColor dstColor)
{
    FILE *pFile = 0;
    int status = 0;
...
int Image::Read(FILE *pFile, ImageColor dstColor)
{
    int status = 0;
...
int Image::Write(const char *pFileName, ImageType dstType, bool bAppend)
{
    FILE *pFile = 0;
    int status = 0;
...
int Image::Write(FILE *pFile, ImageType dstType)
{
    int status = 0;
    ...
    if(dstType == IT_RAW)
    {
        ...
        {
            if(iLine != fwrite((m_pPointer + m_iStep*i), 1, iLine, pFile))
            {
                status = -1;
                break;
            }
...
int Image::Convert(ImageColor dstColor, Image *pDstImage)
{
    int  status;
    ...
    if(!pDstImage && pTmpBuffer)
        _aligned_free(pTmpBuffer);

    return status;
}

int Image::CopyTo(Image *pDstImage)
{
    unsigned char *pSrcRow;
    unsigned char *pDstRow;
...
int Image::FillBorder(BorderType borderType, int iConstValue)
{
    unsigned char *pSrcRow;
    unsigned char *pDstRow;
...
int Image::Generate(ImageGen imageGen)
{
    if(!IsInitialized(this))
        return -1;
...
int Image::DrawPixel(unsigned int iX, unsigned int iY, unsigned char color[4])
{
    if(!m_pPointer)
        return -1;
...
int Image::DrawRect(Rect rect, unsigned char color[4], unsigned int iThickness)
{
    int i, j;
...
int Image::DrawLine(Vect vect, unsigned char color[4])
{
    if(!m_pPointer)
        return -1;
